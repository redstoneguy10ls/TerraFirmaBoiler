    ...
    private void init(final FMLCommonSetupEvent event) {
        event.enqueueWork(() -> {
            registerHeater(TFCBlocks.CHARCOAL_FORGE.get(), (level, pos, state) -> {
                ...
                LOGGER.debug(String.valueOf(2 * Config.charcoalForgeHeaterLevelScaleConfigEntry.get().floatValue()));
                if (heat < 3) return -1;
                if (heat < 7) return 1 * Config.charcoalForgeHeaterLevelScaleConfigEntry.get().floatValue();
                return 2 * Config.charcoalForgeHeaterLevelScaleConfigEntry.get().floatValue();
            });
            registerHeaterProvider((level, pos, state) -> {
                if (ActiveBoilerHeaterBlockTag.matches(state)) {
                    return (level1, pos1, state1) -> Config.activeBoilerHeaterLevelConfigEntry.get().floatValue();
                }
                return null;
            });
        });
